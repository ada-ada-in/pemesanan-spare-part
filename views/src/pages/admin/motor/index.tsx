...
export async function getServerSideProps(context: GetServerSidePropsContext) {
  ...
  const [dataMotor] = await Promise.all([
    getData(token, `${backendUrl}/motor`),
  ]);
  return {
    props: {
      token,
    ...
    if (data) {
      ...
      if (searchTerm) {
        filtered = filtered.filter((callback) => {
          const { motor_name } = callback;
          return motor_name.toLowerCase().includes(searchTerm.toLowerCase());
        });
      }
  ...
  return (
    <>
      <Sidebar profile={profile}>
        <Table
          title="Motor"
          description="Management data motor pada Yamaha Sabang Raya
        Motor Handil."
          link="/admin/motor/add"
          data={data}
        >
                              ...
                              >
                                <div className="sm:flex sm:items-start">
                                  ...
                                  <div className="mt-3 text-center sm:ml-4 sm:mt-0 sm:text-left">
                                    ...
                                    >
                                      Yakin ingin menghapus data Motor{" "}
                                      {`${motorName} tahun ${tahun}`}
                                    </DialogTitle>
                                    <div className="mt-2">
